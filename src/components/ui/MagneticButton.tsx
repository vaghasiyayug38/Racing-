...
export default function MagneticButton({ children, variant = 'primary', size = 'md', className = '', ...props }: MagneticButtonProps) {
  ...
  const baseClasses = "relative overflow-hidden font-orbitron font-bold uppercase tracking-wider transition-all duration-300 rounded-full border-2"
  const sizeClasses = size === 'sm' ? 'px-6 py-2 text-sm' : size === 'lg' ? 'px-10 py-4 text-lg' : 'px-8 py-3'
